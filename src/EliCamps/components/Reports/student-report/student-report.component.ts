...
import { STUDENT_COL_DEFS, AIRPORT_REPORT_COL_DEFS, SITE_BY_DATE_REPORT_COL_DEFS } from 'src/EliCamps/common/elicamps-column-definitions';
import { AllCommunityModules } from '@ag-grid-community/all-modules';
import { Router } from '@angular/router';
import { ChipRendererComponent } from 'src/EliCamps/ag-grid/renderers/chip-renderer/chip-renderer.component';
import { Student, Campus, Room, ProgrameAddins, HomeStay } from 'src/EliCamps/EliCamps-Models/Elicamps';
...
export class StudentReportComponent implements OnInit {
  ...
  public getCampusList = async () => {
    ...
    const addinList = await this.listService.getAllAddins(params).toPromise().catch(error => throwError(error));
    if (addinList) {
  ...
  public getAgentList = () => {
    ...
    this.listService.getInsuranceReport().subscribe(res => {
      this.studentList = res.map(row => ({
        ...
        homestayName: this.getHomeStay(row.homestayID) ? this.getHomeStay(row.homestayID).name : '',
        programeAddins: this.getAddins(row.programeAddins),
        hostContactInfo: this.getHomeStay(row.homestayID) ? this.getHomeStay(row.homestayID).cellNumber : '',
  ...
  public getHomeStay = (homestayID: number) => {
    if (homestayID) {
      const homestay = this.homestayList.find(home => home.homeId === homestayID);
      if (homestay) {
        return homestay;
      }
    }
